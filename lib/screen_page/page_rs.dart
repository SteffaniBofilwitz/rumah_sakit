...
class _PageRumahSakitState extends State<PageRumahSakit> {
  ...
  Future<void> fetchRumahSakit() async {
    final response = await http.get(Uri.parse('http://192.168.100.44/rumah_sakit/getRS.php?id_kabupaten=${widget.idKabupaten}'));
            ...
            return Card(
              child: ListTile(
                leading: Image.network(
                  'http://192.168.100.44/rumah_sakit/gambar/${rumahSakit.gambar}', // Menggunakan URL dari database API
                  width: 50,
                  height: 50,
                  fit: BoxFit.cover,
                ),
